...
class ResizeHandle {
  constructor(options = {}) {
    logger.debug('ResizeHandle 생성자 시작');
    ...
    logger.debug(`설정된 옵션: containerId=${this.options.containerId}, handleId=${this.options.handleId}`);

    // 요소 참조
    this.container = getElement(this.options.containerId);
    this.handle = getElement(this.options.handleId);
    
    logger.debug(`요소 참조 결과: container=${!!this.container}, handle=${!!this.handle}`);
    ...
    if (this.container && this.handle) {
      ...
      logger.error(`리사이즈 핸들 컴포넌트 초기화 실패: 필요한 요소를 찾을 수 없음 (container: ${!!this.container}, handle: ${!!this.handle})`);
      
      // 지연 초기화 시도
      logger.debug('지연 초기화 시도 예약 (500ms)...');
      setTimeout(() => {
        this.container = getElement(this.options.containerId);
        this.handle = getElement(this.options.handleId);
        
        logger.debug(`지연 요소 재시도: container=${!!this.container}, handle=${!!this.handle}`);
        
        if (this.container && this.handle) {
          this.init();
          logger.log('리사이즈 핸들 컴포넌트 지연 초기화 성공');
        } else {
          logger.error('리사이즈 핸들 컴포넌트 지연 초기화도 실패: DOM 요소 누락');
        }
      }, 500);
    }
  }
